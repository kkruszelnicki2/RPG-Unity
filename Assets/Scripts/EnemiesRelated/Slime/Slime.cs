...
public class Slime : Enemy
{
    ...
    void Update()
    {
        if(Vector2.Distance(Player.transform.position,transform.position) < sightRange && !isDamaged) //je�eli gracz znajduje si� w polu widzenia przeciwnika
        {
            //sprawdzanie po�o�enia przeciwnika wzgl�dem gracza
            ...
            transform.Translate(direction.normalized * speed * Time.deltaTime); //ruch przeciwnika (normalizacja wektora, uwzgl�dnienie czasu oraz pr�dko�ci)
        }
        else if(isDamaged)
        {
    ...
    //Odepchni�cie potwora gdy otrzyma atak
    public void Push()
    {
        if(!isDamaged)
        {
