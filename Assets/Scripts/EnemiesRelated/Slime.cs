...
public class Slime : MonoBehaviour
{
    ...
    void Update()
    {
        if(Vector2.Distance(Player.transform.position,transform.position) < sightRange && !isDamaged) //je¿eli gracz znajduje siê w polu widzenia przeciwnika
        {
            ...
            //ruch przeciwnika
            transform.localScale = new Vector3(-1 * Mathf.Abs(transform.localScale.x) * direction.x / Mathf.Abs(transform.position.x - Player.transform.position.x), transform.localScale.y, 1); //obracanie spritem przeciwnika gdy idzie w lewo/prawo
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if (collision.gameObject.name == "attackHitbox" && !isDamaged) //Kolizja z broni¹ gracza
        {
            ...
            {
                gameObject.GetComponent<EnemyDeath>().Death(Spawner,Player);
            }
    ...
    public void Constructor(GameObject space,GameObject spawner)
    {
        Place = space;
        Spawner = spawner;
    }
